...
private:
    ...
    void EnsureGeneratedUntil(size_t index) const {
        ...
        size_t attempts = 0;
        const size_t max_attempts = 10000; // Защита от бесконечного цикла
        while ((size_t)data_.GetLength() <= index && attempts < max_attempts) {
            T next = gen->Next();
            next = ApplyMaps(next);
            if (ApplyWheres(next)) {
        ...
        if (attempts >= max_attempts && (size_t)data_.GetLength() <= index) {
            throw std::runtime_error("Failed to generate element: too many filtered elements");
        }
    }
