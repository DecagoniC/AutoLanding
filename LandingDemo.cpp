...
int main() {
    ...
    ShipState initial{};
    initial.pose.position = {0.0, 0.0, 1500.0};
    ...
    log << "Step\tAltitude(z, m)\tVerticalVel(dz/dt, m/s)\tHorizontalVel(sqrt(vx^2+vy^2), m/s)\tPitch(rad)\tRoll(rad)\n";
    ...
    const auto log_state = [&log](const ShipState& s, double horiz_speed) {
        log << "step=" << s.step
    ...
    log_state(state, horizontal_speed(state));
    ...
    for (size_t i = 0; i < max_steps; ++i) {
        ...
        if (state.pose.position.z <= 0.5 &&
            std::abs(state.motion.velocity.z) < 0.5 &&
            std::abs(state.motion.velocity.x) < 0.5 &&
            std::abs(state.motion.velocity.y) < 0.5) {
